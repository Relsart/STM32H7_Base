...
void Mcu::init ()
{
     ...
     */
    {
        ...
        spi1.init(spiconf);
        uint32_t spiFrq = spi1.getSckFreq();
        Log(lmSystem, Info) << "SPI1 speed = " << spiFrq << " Hz";
    }   // using namespace driver::gpio;
    ...
    sdmmc::Config sdCfg
    {
        .clockEdge = sdmmc::ClockEdge::Rising,
        .busWide = sdmmc::BusWide::W4_bits,
        .clockDiv = 4,
        .clockPowerSave = false,
        .hwFlowCtrl = true
    };
    //sdmmc::Result sdResult = sdCard.init(sdCfg);
    ...
    usb::Config usbConf
    {
        .dev_endpoints = 9,
        .dma_enable = 0,
        .speed = usb::UsbDevSpeed::Full,
        .phy_itface = usb::PhyIFace::EMBEDDED,
    };
    usbDev.init(usbConf);
...
Peripheral::Peripheral() : m_Mcu(&mcu)
{}

bool Peripheral::init()
{

    /* Here devices initialisation invokes */
